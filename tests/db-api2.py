...
def conversionTest():
    ...
    if SnakeSQL.Date(2004,12,12) != datetime.date(2004,12,12):
        print("Date failed.", SnakeSQL.Date(2004,12,12), datetime.date(2004,12,12))
    ...
    if SnakeSQL.TimeFromTicks(tt) != td.time():
        print("TimeFromTicks failed.", SnakeSQL.TimeFromTicks(time.time()), datetime.datetime.now().time())
    if SnakeSQL.TimestampFromTicks(tt) != td:
        print("TimestampFromTicks failed.", SnakeSQL.TimestampFromTicks(time.time()), datetime.datetime.now())
...
def typesTest():
    ...
    if not SnakeSQL.BINARY == SnakeSQL._type_codes['BINARY']:
        print("BINARY comparison failed.", SnakeSQL.BINARY.values ,SnakeSQL._type_codes['BINARY'])
...
if __name__ == '__main__':
    ...
    print("DBM Test")
    print("--------")
    connection = SnakeSQL.connect('_testdbm', driver='dbm', autoCreate=True)
    ...
    print("\nCSV Test.")
    print("--------")
    connection = SnakeSQL.connect('_testcsv', driver='csv', autoCreate=True)
    cursor = connection.cursor()
    insertTest()
    updateTest()
    conversionTest()
